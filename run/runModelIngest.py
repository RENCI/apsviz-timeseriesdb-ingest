...
def getSourceMeta(dataType):
    ...
    try:
        ...
        # Run query
        cur.execute("""SELECT data_source, source_name, source_archive, source_variable, source_instance, forcing_metaclass, filename_prefix, location_type, 
                              data_type, units FROM drf_source_model_meta 
                       WHERE data_type = %(datatype)s ORDER BY filename_prefix""", {'datatype': dataType})

        # convert query output to Pandas dataframe
        df = pd.DataFrame(cur.fetchall(), columns=['data_source', 'source_name', 'source_archive', 'source_variable','source_instance','forcing_metaclass', 
                                                   'filename_prefix', 'location_type', 'data_type', 'units'])
...
def runHarvestFile(harvestDir, ingestDir, modelRunID):
    ''' This function runs createHarvestDataFileMeta.py, which creates harvest meta data files, that are ingested into the 
        drf_harvest_model_file_meta table, in the database, by running ingestTasks.py using --inputTask ingestHarvestDataFileMeta.
        Parameters
            harvestDir: string
    ...
    filelist, grid_name, advisory, timemark, forcing_meta_class, storm, instance_name, workflow_type = gdm.getInputFileName(harvestDir,modelRunID)
    ...
    for filenamepath in filelist:
        ...
        # Check if dfcheck is empty, for forecast. If it is a new source is added to the drf_source_model_meta table ,and then drf_gauge_source table. 
        if dfcheck.empty:
            # Log results
        ...
        dfcheck = gdm.checkModelSourceMeta(nowcast_prefix)

        # Check if dfcheck is empty, for nowcast data. If it is a new source is added to the drf_source_model_meta table ,and then drf_gauge_source table.
        if dfcheck.empty:
            # Log results
            logger.info('The following source does not exist in the drf_source_model_meta table:\n '+
